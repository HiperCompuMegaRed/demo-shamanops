...
var (
	...
	colors       = []string{"violet", "yellow", "pink"}
	envLatency   float64
	envErrorRate int
)
...
func main() {
	...
	router.HandleFunc("/secret", getSecretEnv) // New handler for returning SECRET_ENV
...
// New handler function to return the SECRET_ENV value
func getSecretEnv(w http.ResponseWriter, r *http.Request) {
	secretEnv := os.Getenv("SECRET_ENV")
	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(map[string]string{"secret": secretEnv})
}
